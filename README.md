...
The following RRULE parts are parsed but ignored during expansion:

  * `BYWEEKNO` - ISO week number expansion (rarely generated by calendar apps)
  * `BYYEARDAY` - day-of-year expansion (essentially unused in practice)
  * `BYHOUR`, `BYMINUTE`, `BYSECOND` - sub-daily expansion

### Features

  * Scheduled calendar refresh
  * Automatic time zone conversion
  * Recurring events via RRULE, supporting:
    * `FREQ` - daily, weekly, monthly, yearly (and hourly, minutely, secondly)
    * `INTERVAL`, `COUNT`, `UNTIL`
    * `BYDAY` - weekday names (`MO`–`SU`), with optional ordinals (e.g. `2MO`, `-1FR`)
    * `BYMONTH` - filter or expand by month number
    * `BYMONTHDAY` - expand to specific days of the month (negative values count from end)
    * `BYSETPOS` - select the nth occurrence(s) from the expanded set
    * `WKST` - week start day
...
`org-ics-import-recurring-future-count` controls how many future occurrences are generated per recurring event (default: 10).
